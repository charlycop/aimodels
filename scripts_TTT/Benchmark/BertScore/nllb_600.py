...
def batch_bert_score(inputs, references, model_name, batch_size=32):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModel.from_pretrained(model_name).to(device)
    
    scorer = BERTScorer(model_type=model_name, num_layers=1, batch_size=batch_size, device=device)
    
    all_p = []
    all_r = []
    all_f1 = []
    
    wandb.init(project="bertscore-nllb600", name="evaluation")
